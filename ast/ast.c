...
static char* let_statement_string(Node* n) {
    ...
    char* value_str = n->as.let_statement.value 
                        ? node_string(n->as.let_statement.value) 
                        : strdup("");

    size_t size = strlen(n->token.literal) + 1 + strlen(name_str) + 3 + strlen(value_str) + 2; 
    char* out = malloc(size);
    snprintf(out, size, "%s %s = %s;", 
             n->token.literal, name_str, value_str);
...
static char* return_statement_string(Node* n) {
    char* val_str = n->as.return_statement.return_value
                        ? node_string(n->as.return_statement.return_value)
                        : strdup("");

    size_t size = strlen(n->token.literal) + 1 +
                  strlen(val_str) + 2; // "return value;"
    char* out = malloc(size);
